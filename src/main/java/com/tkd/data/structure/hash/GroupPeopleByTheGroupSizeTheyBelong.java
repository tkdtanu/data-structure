 ...
 *
 */
public class GroupPeopleByTheGroupSizeTheyBelong {
	...
	public static List<List<Integer>> groupThePeople(int[] groupSizes) {
		...
		Map<Integer, List<Integer>> mapOfGroupSizeAndIndex = new HashMap<>();
		...
		for (int i = 0; i < n; i++) {
			...
			final int index = i;
			mapOfGroupSizeAndIndex.compute(groupSize, (k, v) -> {
				if (v == null) {
					v = new ArrayList<>(groupSize);
				}
				v.add(index);
